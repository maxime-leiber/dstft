...
class DSTFT(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "eps",
            torch.tensor(
                torch.finfo(torch.float).eps, dtype=self.dtype, device=self.device
            ),
        )
        self.register_buffer(
            "min",
            torch.tensor(
                torch.finfo(torch.float).min, dtype=self.dtype, device=self.device
            ),
        )
        ...
        self.strides = nn.Parameter(
            torch.full(stride_size, abs(stride), dtype=self.dtype, device=self.device),
        ...
        self.win_length = nn.Parameter(
            torch.full(
                win_length_size, abs(win_length), dtype=self.dtype, device=self.device
            ),
        ...
        self.win_pow = nn.Parameter(
            torch.full(
                win_pow_size, abs(win_pow), dtype=self.dtype, device=self.device
            ),
    ...
    def __pow_transform(self, p_in):  # born stride entre 0 et 2N
        p_out = torch.minimum(
            ...
            torch.full_like(p_in, self.pow_max, dtype=self.dtype, device=self.device),
        )
        return p_out
    ...
    def effective_strides(self):
        ...
        cat = (
            torch.cat(
                (
                    torch.tensor([self.N], dtype=self.dtype, device=self.device),
    ...
    def stft(self, x: torch.tensor, direction: str):
        ...
        shift = torch.arange(
            end=self.N, device=self.device, dtype=self.dtype, requires_grad=False
        )
    ...
    def stride(self, x) -> torch.tensor:
        ...
        idx_floor = idx_floor.long()[:, None].expand((self.T, self.N)) + torch.arange(
            0, self.N, device=self.device
        )
    ...
    def window_function(self, direction: str, idx_frac) -> torch.tensor:
        if self.tapering_function not in {
            "hann",
            "hanning",
        }:
            raise ValueError(
        ...
        if self.tapering_function == "hann" or self.tapering_function == "hanning":
            if direction == "forward":
                ...
                d_tap_win = (
                    -pi / self.actual_win_length.pow(2) * ((self.N - 1) / 2 - base) * f
                )
                d_tap_win[mask1] = 0
                d_tap_win[mask2] = 0
    ...
    def print(
        ...
        if self.tap_win is not None and wins is True:
            ...
            ax.plot(
                self.T + 0.5 + x.squeeze().cpu().numpy(),
                linewidth=1,
            )
            for i, start in enumerate(self.frames.detach().cpu()):
                ax.plot(
                    range(
                        int(start.floor().item()), int(start.floor().item() + self.N)
                    ),
                    self.T
                    - i
                    - 1.3
                    + 150
                    * self.tap_win[:, i, :, :].mean(dim=1).squeeze().detach().cpu(),
                    c="#1f77b4",
                )
...
class FDSTFT(nn.Module):
    ...
    def stft(self, x: torch.tensor, direction: str):
        ...
        # print(spectr.shape)
        shift = torch.arange(
            end=self.F, device=self.device, dtype=self.dtype, requires_grad=False
        )
        shift = idx_frac[:, None] * shift[None, :]  # T, N
