...
#    def coverage(self):            
#        # compute coverage
#        print(self.effective_strides, self.actual_win_length, self.actual_strides)        
#        coverage = torch.minimum(self.effective_strides[1:], self.actual_win_length.expand((self.T))[:-1]) 
#        print(coverage)
#        coverage = torch.cat((coverage, self.actual_win_length[-1].expand((self.T))), dim=0).squeeze()
