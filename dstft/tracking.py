...
def frequency_tracking(y, fs, spec, fmin, fmax, alpha, orders=[]):
    ...
    Prob[(freqs >= fmin) & (freqs <= fmax)] = spec_db[
    ...
    for o in orders:
        ...
        idx = torch.arange(0, spec.shape[0], device=y.device)[
            (freqs >= fmin) & (freqs <= fmax)
        ]
    ...
    Prior = 0.5 * (
        freqs_centred.repeat(T, 1).T / sigma.repeat(L, 1)
    ) ** 2 - torch.log(np.sqrt(2 * math.pi) * sigma.repeat(L, 1))

    out = dynamic_programming(Prob.squeeze(), Prior)
    # print((out[None, None, ...]*df).shape)

    # Estimated frequency
    fest = torch.nn.functional.interpolate(
        (out * df)[None, None, None, ...], size=y.shape[-1], mode='bicubic'
    )
...
if __name__ == '__main__':
    ...
    for i in range(num_harmoniques):
        x += (
            1
            / (i + 1)
            * torch.sin(2 * (i + 1) * pi * torch.cumsum(f, dim=0) / fs)
        )
    ...
    f_hat, out = frequency_tracking(
        y=x, fs=fs, spec=spec, fmin=0, fmax=0.25, alpha=100
    )
